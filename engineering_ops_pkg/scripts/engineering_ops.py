...
class EngineeringOperations:
    """
        This class represents the engineering operations module specified in UMAA.

        'EngineeringOperations' handles observing the overall wellbeing of the UUV such as fault detection, 
        status monitor, and fuel gages. Manages decision-making associated with the vehicle hardware and 
        software to include optimizing use of the vehicle resources
    """

    _ENGINEERING_OPS_MSG = EngineeringOperationsMsg()
    
    def __init__(self):
        """
            Constructs an 'EngineeringOperations' object. In constructing a 'EngineeringOperations'
            object you are creating a ROS node which handles status monitoring of the UUV.
            ...
            pub_logger: A ROS publisher which broadcasts logging info about 'EngineeringOperations' object
            ...
            _ENGINEERING_OPS_MSG: An 'EngineeringOperationsMsg' that belongs to 'EngineeringOperations' class 
                    for updating and publishing

            How Things Work:
            +--------------------+
            1. 'EngineeringOperations' listens to only 'AutonomyBus'
            2. The data from 'AutonomyBus' is a big blob of data comprised of all other module's/package's
                    information
            3. The blob of data is parsed for the data needed for 'EngineeringOperations'
            4. 'data' field is a dictionary of (key: module_name, value: module_name_message_type)
            5. 'data' should at least store an 'autonomy_bus', 'engineering_ops', and other module names
                    pertinent to 'engineering_ops' as keys
                - e.g., 'data = {'autonomy_bus': AutonomyBusMsg, 'engineering_ops': EngineeringOperationsMsg,
                'maneuver_ops': ManeuverOperationsMsg}', in this case 'EngineeringOperations' 
                relies on data from 'maneuver_ops_pkg'
        """
        # Create a ROS node named, "engineering_ops"
        rospy.init_node('engineering_ops', anonymous=True)
        # TODO: Setup publisher of data
        #self.pub_data = rospy.Publisher('engineering_ops_out', _, queue_size=10)
        # Setup publisher of logging info
        self.pub_logger = rospy.Publisher('engineering_ops_logger_out', String, queue_size=10)
    ...
    def _publish(self):
        ...
        while not rospy.is_shutdown():
            # Store 'EngineeringOperationsMsg' in 'data'
            self.data['engineering_ops'] = self._ENGINEERING_OPS_MSG
            # Update timestamp of 'EngineeringOperationsMsg' before every publication
            self._ENGINEERING_OPS_MSG.header.stamp = rospy.Time.now()
            # Create logger message
            logger_msg = '[{0}] "engineering_ops" node is okay!'.format(rospy.Time.now())
            # TODO: Publish "EngineeringOperationsMsg" to "engineering_ops_out" topic
            #self.pub_data.publish(_)
            # Publish "logger_msg" to "engineering_ops_logger_out" topic
            self.pub_logger.publish(logger_msg)
            # Sleep for 1 second
            rate.sleep()
        print('"engineering_ops" node has gone offline.')
            
    def _listen_data(self):
        """
            Setup 'EngineeringOperations' to listen other module's/package's actual data info
        """
    ...
    def _autonomy_bus_logger_callback(self, data):
        ...
        """
        # For debugging
        rospy.loginfo('Engineering operations receiving: [{0}] from /autonomy_bus_logger_out'.format(data.data))
        # Populate logging data pertaining to the autonomy bus
        self.data_logger['autonomy_bus_log'] = data.data

def main():
    """
        Driver method for 'engineering_ops.py'
    """
    # Start 'CommunicationOperations'
    engineering_ops = EngineeringOperations()
